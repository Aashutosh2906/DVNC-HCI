...
const CONFIG = {
    ...
    neuralParticles: 30, // Number of particles for the neural network background
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    initNeuralCanvas(); // Initialize the neural network canvas
});
...
function initializeElements() {
    elements = {
        ...
        neuralCanvas: document.getElementById('neuralCanvas'), // Get the canvas element
    };
}
...
// --- SUBTLE CANVAS BACKGROUND (Neural Network) ---
...
function initNeuralCanvas() {
    ...
    // Resize canvas to fill window and re-initialize particles
    const resizeCanvas = () => {
        canvas.width = window.innerWidth;
        canvas.height = window.innerHeight;
        neuralParticles = []; // Clear and re-create particles on resize
    ...
    animateNeuralNetwork(); // Start the animation loop
}
...
function animateNeuralNetwork() {
    ...
    // Determine particle and line colors based on current theme
    ...
    for (let i = 0; i < neuralParticles.length; i++) {
        for (let j = i; j < neuralParticles.length; j++) {
            ...
            if (distance < 150) { // Max distance for a connection
                ctx.beginPath();
    ...
    requestAnimationFrame(animateNeuralNetwork); // Loop the animation
}
